...
const officialConsonants = {
  p: 'p-sound', b: 'b-sound', t: 't-sound', d: 'd-sound', k: 'k-sound', g: 'g-sound',
  f: 'f-sound', v: 'v-sound', s: 's-sound', z: 'z-sound', h: 'h-sound', m: 'm-sound',
  n: 'n-sound', ng: 'ng-sound', l: 'l-sound', r: 'r-sound',
  'th-voiceless': 'es/theta-sound', 'th-voiced': 'es/eth-low-sound',
  sh: 'es/zh1-sound', zh: 'es/zh2-sound', ch: 'es/ch-sound',
  'j-sound': 'es/jya-sound', y: 'es/yot-sound', w: 'es/w-sound'
};

const fullWordTeaching = {
  tr: 'tree', dr: 'dream', ts: 'cats', dz: 'beds'
};

const localSingle = {};
Object.keys(single).forEach(function (id) {
  if (!officialConsonants[id]) localSingle[id] = single[id];
});

const localComposites = {};
Object.entries(composites).forEach(function (entry) {
  const first = entry[1][0];
  const second = entry[1][1];
  const hasFirst = Boolean(localSingle[first] || auxiliarySources[first]);
  const hasSecond = Boolean(localSingle[second] || auxiliarySources[second]);
  if (hasFirst && hasSecond) localComposites[entry[0]] = entry[1];
});
...
async function main() {
  ...
  const allSources = Object.assign({}, localSingle, auxiliarySources);
  ...
  const manifest = {};
  for (const id of Object.keys(localSingle)) {
    const sourceName = localSingle[id];
  ...
  for (const [id, recipe] of Object.entries(localComposites)) {
  ...
  for (const [id, folder] of Object.entries(officialConsonants)) {
    const sourceUrl = 'https://soundsofspeech.uiowa.edu/assets/phonemes/' + folder + '/examples/sound.mp4';
    const localFile = path.join(outputDir, id + '.wav');
    ...
    manifest[id] = {
      ...
      shared: true,
      kind: 'human',
      credit: 'Sounds of Speech, University of Iowa - independent phoneme demonstration',
      sourceUrl: sourceUrl,
      license: 'Official educational demonstration linked directly from the University of Iowa'
    };
  }

  for (const [id, word] of Object.entries(fullWordTeaching)) {
    const localFile = path.join(outputDir, id + '.wav');
    if (fs.existsSync(localFile)) fs.unlinkSync(localFile);
    const uk = icibaUrl(word, 'uk');
    const us = icibaUrl(word, 'us');
    manifest[id] = {
      uk: uk,
      us: us,
      shared: true,
      kind: 'human-word',
      label: word,
      credit: 'Kingsoft Iciba human dictionary recording',
      sourceUrl: [uk, us],
      license: 'Used by direct reference from Kingsoft Iciba'
    };
  }
